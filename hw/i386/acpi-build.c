...
enum { PCI, PCIE };
static void init_pci_acpi(Aml *dev, int uid, int type, bool enable_native_pcie_hotplug)
{
    if (type == PCI) {
        ...
        aml_append(dev, aml_name_decl("_UID", aml_int(uid)));
        aml_append(dev, build_q35_osc_method(enable_native_pcie_hotplug));
    }
}
           ...
           Range *pci_hole, Range *pci_hole64, MachineState *machine)
{
    ...
    if (bus) {
        QLIST_FOREACH(bus, &bus->child, sibling) {
            ...
            init_pci_acpi(dev, bus_num, pci_bus_is_express(bus) ? PCIE : PCI, true);
